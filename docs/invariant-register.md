...
`packages/ui/src/compose/controls/spec.ts:133`
...
`packages/ui/src/compose/controls/spec.ts:263`
...
`packages/ui/src/config/store.ts:530`
...
`packages/ui/src/config/store.ts:1095`
...
`packages/ui/src/config/store.ts:415`
...
`packages/ui/src/config/types.ts:429`
...
`packages/ui/src/config/types.ts:299`
...
`packages/ui/src/config/store.ts:1305`
...
`packages/ui/src/config/store.ts:1044`
...
`packages/ui/src/config/store.ts:1452`
...
`packages/ui/src/config/store.ts:1674`
...
`packages/ui/src/config/store.ts:1062`
...
`packages/ui/src/config/store.ts:624`
...
`packages/ui/src/config/store.ts:385`
...
`packages/ui/src/config/store.ts:474`
...
`packages/ui/src/config/store.ts:1625`
...
`packages/ui/src/config/store.ts:193`

### `config/screen-spacing-in-range-on-step` — rung 6

**Mechanism.** choke-point — `sanitizeScreenSpacing` is the only producer of a stored ScreenSpacing: the untrusted overlay boundary (config/parse.ts parseScreens) and the store writer (config/store.ts setScreenSpacing) both call it, and nothing else writes `screens.spacing`. Corrupt input becomes a safe value or is dropped (the clampRect precedent, shell/panelCanvas.ts) — never a throw

**Why.** the gutter is subtracted from the card's drawn box (span·u − gutter), so an off-grid or absurd gutter breaks the quantum every card is sized on; the SD file is hand-editable JSON

**Debt — promotion.** promote to rung 7 by branding ScreenSpacing so a hand-written literal is not assignable. Blocked on the brand surviving JSON round-trips to the SD card — the same block as ShapingConfig's Envelope @debt.

`packages/ui/src/config/screenSpacing.ts:10`
...
`packages/ui/src/config/store.ts:1370`
...
`packages/ui/src/config/store.ts:926`
...
`packages/ui/src/config/store.ts:780`
...
`packages/ui/src/config/store.ts:435`
...
`packages/ui/src/shell/panelCanvas.ts:923`
...
`packages/ui/src/shell/panelCanvas.ts:1822`
...
`packages/ui/src/shell/panelCanvas.ts:1791`
...
`packages/ui/src/shell/panelCanvas.ts:1297`
...
`packages/ui/src/shell/panelCanvas.ts:1309`
...
**Mechanism.** source assertion — test/intrinsic-floors.test.ts walks every declaration block in this sheet, selects the ones that declare a `height` AND clip (`overflow`/`overflow-y: hidden`), and fails the suite unless each one's cascade also declares a block-axis floor: a non-auto non-zero `min-height`, or `flex-shrink: 0` / `flex: 0 0` where not shrinking IS the intent. The scan asserts its own catch (>= 10 matching rules) so a scanner that has stopped matching fails loudly instead of passing everything, and the four Shaping prose rows are additionally pinned BY NAME, read through their shared cascade rather than per block, so a guard dropped from one of the four that are known to need it is its own failing test rather than one line in a list. It is a SOURCE assertion, not a measurement: it cannot say a floor is the RIGHT number, only that a fixed-height clipped box is not written with no floor at all

**Why.** #136: `.shp-caveat` is the only shrinkable child of the Sweep card's body, and a flex child whose overflow is not `visible` has an automatic minimum size of ZERO — so it absorbed the card's whole deficit alone and rendered at 0px at the card's own registry size (measured 2026-08-28 headless: 32px at rowSpan 200, 4px at 124, 0px at the coded pin 118). Its two declared lines were invisible on every browser, not only after a resize. The second-order damage is why this is a rule and not a one-line fix: contentRowSpan sums each child's RENDERED height, so a child that shrinks with the card makes the card's own floor a function of the card's own size — measured on this card, rowStop 129 at rowSpan 200 and 121 at rowSpan 118. A floor enforced from that measurement enforces a number the shrink itself moved, which is the hysteresis card-floor-independent-of-size exists to catch. Six rules in this sheet were unguarded when the scan was written, and the same shape had been written four separate times before anybody measured it. The row was DEFERRED on GIT_136 rather than skipped: the ratchet stood at 24 against a ceiling of 25 and a third +1 was not that branch's number to rule on (Gabe, 2026-08-28: "give it the row and bump the ceiling to 27")
