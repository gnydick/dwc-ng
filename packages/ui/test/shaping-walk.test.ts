...
test("a known stage states its numbers so they can be checked", () => {
	...
	const r: ToolResults = { ...emptyResults(0), fingerprint: fp, captures: new Array(40).fill(null).map((_, i) => ({ file: `c${i}.csv`, axis: "X" as const, dir: "+" as const, rep: i, fit: mode(38.66), tStop: null })) };
...
test("a stage that RAN but is caveated does not read as finished", () => {
	...
	const w = walkThrough({ ...emptyResults(0), fingerprint: fp }, p);
...
test("the walk is ordered by the workflow, not by severity across stages", () => {
	...
	const w = walkThrough({ ...emptyResults(0), fingerprint: fp, candidates: [] as never[] }, p);
	assert.equal(w.next?.step, "measure");
});
