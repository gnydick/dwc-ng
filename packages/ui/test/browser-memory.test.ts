...
test("the open file round-trips per root, and closing is storable", () => {
	...
	assert.equal(m.file, "0:/macros/tools/park.g");
	assert.equal(m.dir, "0:/macros/tools", "file and dir coexist");
	assert.equal(loadBrowserMemory("0:/sys").file, undefined, "another root is unaffected");
	saveBrowserFile("0:/macros", null);
	assert.equal(loadBrowserMemory("0:/macros").file, undefined, "closing is remembered");
	assert.equal(loadBrowserMemory("0:/macros").dir, "0:/macros/tools", "closing kept the directory");
});
