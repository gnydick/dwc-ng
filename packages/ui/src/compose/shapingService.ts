/**
 * The Shaping Lab's service, on the far side of the dynamic boundary.
 *
 * IT USED TO LIVE IN compose/services.ts, and that was the defect (#126). The
 * service registry is reached on every cold load — `main.tsx` -> `App.tsx` ->
 * `Shell.tsx` -> `ComposedScreen.tsx` -> `compose/services.ts` — so a factory
 * written there imports its dependencies at module scope on the critical path,
 * whatever screen the operator is actually on. `cards/ShapingCards.tsx` has
 * been correctly lazy the whole time; the SERVICE defeated it behind its back.
 * Measured on `uat` b169582: 23 modules of `src/shaping/**`, 189,970 source
 * bytes, 21,635 B minified, on every board load of a screen that tunes a
 * machine rather than runs one — and 4.7x the amount by which that tree
 * exceeded the eager budget and could not be deployed at all.
 *
 * This is the SECOND time the same shape has been fixed. The first
 * (2026-08-24) moved the `products` memo off this service and removed one
 * tendril; `shaping/selection.ts` (GIT_108) and `shaping/preconditions.ts`
 * (GIT_51) grew back onto it because nothing stood between the eager registry
 * and this subsystem. Something does now: the eager import graph is walked in
 * `test/lazy-bundle.test.ts`, and a `shaping/**` module reaching it fails the
 * suite by name.
 *
 * HOW IT IS REACHED. `cards/ShapingCards.tsx` re-exports `shapingService`, so
 * this module lands in the Lab's ONE chunk rather than a second one the board
 * would have to serve separately, and `compose/cards.tsx`'s `loadShapingLab`
 * hands the export to `provideShapingService` after awaiting that chunk and
 * before returning a component to Solid. So the factory is registered before
 * any body can render, and therefore before any body can call
 * `ctx.service("shaping")` — the ordering is a property of the loader rather
 * than something a call site has to get right, and a dropped re-export is a
 * compile error rather than a screen that throws.
 *
 * WHAT IS NOT HERE: the accelerometer's four members. `shaping/accelService.ts`
 * carries those, because the EAGER Settings card uses them too and reaching
 * them through this service is what dragged the Lab onto the critical path in
 * the first place. Both sides take the same pool entry, so there is still one
 * instance per screen.
 *
 * Nothing about WHAT the service does changed when it moved.
 */
import { createEffect, createMemo, createSignal, onCleanup } from "solid-js";
import { createStore } from "solid-js/store";
import { createShapingStore, verifyAnalysis } from "../shaping/store.ts";
...
import { accelAddrFor } from "./accelService.ts";
 ...
 */
export function shapingService(base: ServiceBaseCtx) {
	 ...
	 *
	 * The SAME derivation the accel service uses (shaping/accelService.ts), not
	 * a second `parseAccelAddr` call: one address, two readers. Internal — a
	 * card that wants the address asks `ctx.service("accel")` for it, so there
	 * is one answer on the screen rather than one per service.
	 */
	const accelFor = (n: number): ReturnType<typeof accelAddrFor> => accelAddrFor(base, n);
...
/**
	...
	// The accelerometer's rate and resolution used to live here. They are
	// `ctx.service("accel")` now (shaping/accelService.ts): the EAGER Settings
	// card needs them, and reaching them through THIS service is what dragged
	// the whole Lab onto every cold load (#126).
	...
	return {
		...
		gate, macroFor, toggleMacro, rank, ranking, problem, offer, runStep,
