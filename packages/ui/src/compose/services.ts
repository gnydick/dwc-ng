...
import { createAccelService } from "./accelService.ts";
import { shapingServiceFactory } from "./shapingSlot.ts";
...
export const SERVICES = {
	...
	/**
	 * The accelerometer's rate and resolution, per tool.
	 *
	 * Its own entry rather than four members of `shaping` below, because the
	 * EAGER Settings card uses it and the Lab does not have to be loaded for
	 * that card to work — which is the whole of #126. Still ONE instance per
	 * screen, so the Settings card and the Lab's Capture card cannot hold two
	 * different ideas of what the sensor reported.
	 */
	accel: (base: ServiceBaseCtx) => createAccelService(base),
	/**
	 * The Shaping Lab, built from the factory its own chunk provides.
	 *
	 * NOT a static import: naming `compose/shapingService.ts` here is exactly
	 * what put 23 modules of `shaping/**` on every cold load (#126). The type
	 * still resolves — `typeof import(...)` in `shapingSlot.ts` is erased — so
	 * `ServiceInstance<"shaping">` is as precise as it ever was, and the pool
	 * below is still the sole owner of the instance.
	 */
	shaping: (base: ServiceBaseCtx) => shapingServiceFactory()(base),
} as const;
