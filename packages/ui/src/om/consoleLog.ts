...
/** Pixel placement + size of the floating console tile. */
export interface ConsoleGeometry {
	left: number;
	top: number;
	width: number;
	height: number;
...
const GEOMETRY_KEY = "dwc-ng.console.geometry";

/** No stored geometry means "not dragged/resized yet" — caller falls back to the CSS default. */
export function loadConsoleGeometry(): ConsoleGeometry | null {
	...
	try {
		...
		if (
			...
			|| typeof (parsed as ConsoleGeometry).left !== "number"
			|| typeof (parsed as ConsoleGeometry).top !== "number"
			|| typeof (parsed as ConsoleGeometry).width !== "number"
			|| typeof (parsed as ConsoleGeometry).height !== "number"
		) return null;
		return parsed as ConsoleGeometry;
	} catch {
		return null;
	}
}

export function saveConsoleGeometry(geometry: ConsoleGeometry): void {
	if (typeof localStorage === "undefined") return;
	try {
