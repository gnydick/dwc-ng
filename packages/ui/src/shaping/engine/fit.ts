 ...
 *          exports no function that returns a measured envelope. The decay
 *          CHART (`charts/decayData.ts`) therefore draws `modeEnvelope` of the
 *          very Mode printed beside it: not a curve that agrees with the fit,
 *          the fit itself, plotted
...
/**
 * Where the fitter looks, as numbers rather than literals scattered through it.
 *
 * They are here because the decay CHART states them to the operator — "under
 * 0.15 s of samples follow the stop" is only true while the chart's 0.15 and
 * the fitter's 0.15 are the same 0.15. A bound may exist in exactly one place.
 *
 * The ACCEPTANCE rule is deliberately not in this table: {@link DECAY_FLOOR},
 * {@link MIN_CYCLES} and {@link MAX_FIT_ZETA} own it, each with the measurement
 * that set it. Two homes for one bound is the drift this table exists to stop,
 * so it must not grow a second copy of them.
 */
export const FIT_DEFAULTS = {
	...
	minWindowS: 0.15,
	/** Head of the window searched for the "did this axis ring at all" peak (s). */
	headS: 0.1,
	/** Dead time after the stop before the analysis region opens (s). */
	leadS: 0.01,
} as const;

/**
 * The analysis region: the half-open sample range `[i0, i1)` the fit is taken
 * over, or `null` when fewer than {@link FIT_DEFAULTS.minWindowS} of samples
 * follow the stop — the one condition under which there is nothing to analyse.
 *
 * This used to carry a sampled band-mask envelope and the two indices bounding
 * a log-slope regression over it, and the chart drew that envelope. GIT_33
 * deleted the measurement (see the header): there is no measured envelope left
 * to hand out, and the one the chart draws is `modeEnvelope`, which cannot
 * disagree with the fit because it IS the fit. Only the region survived — and
 * it survived because the fitter and the chart still have to agree on where it
 * starts, which is the whole of what the invariant below now guards.
 *
 * @invariant one-decay-window
 * @rung 6  choke-point — `decayWindow` is the only place the analysis region is
 *          located. `fitDecay` fits this range and nothing else, and
 *          `charts/decayData.ts` frames, band-passes and lays its envelope over
 *          this range and nothing else, so sample 0 of the drawn envelope is
 *          sample 0 of the fitted data by construction rather than by two
 *          arithmetics that happen to agree
 * @why the chart exists to let an operator see WHY a fit came out as it did; a
 *      curve drawn over a region half a cycle away from the fitted one sits off
 *      the ring it claims to describe, and the picture would be believed
 * @debt the region is handed out as two plain integers, so a future caller
 *       could frame one capture's region around another capture's Mode. Promote
 *       by returning the Mode|NoFit alongside it from a single call, once a
 *       second consumer exists that needs both. Tracked with the E1 work item
 */
export type DecayWindow = {
	/** First sample of the analysis region, as an index into the input array. */
	readonly i0: number;
	/** One past the last, clamped to the end of the capture. */
	readonly i1: number;
};
...
export function decayWindow(axis: Float64Array, rate: Hz, tStop: Seconds, opts: FitOptions = {}): DecayWindow | null {
	...
	const i0 = Math.round((tStop + FIT_DEFAULTS.leadS) * rate);
...
/**
 * The mean-removed samples of an analysis region — the exact array the fit is
 * taken over, and the exact array the chart band-passes to draw the ring.
 *
 * Exported so `charts/decayData.ts` cannot arrive at a different one: the fit
 * removes the region's mean before it looks at anything, and a chart that
 * band-passed the raw slice instead would draw a trace offset from the fitted
 * data by whatever DC the accelerometer had that second.
 */
export function decaySegment(axis: Float64Array, w: DecayWindow): Float64Array {
	const seg = axis.slice(w.i0, w.i1);
	let mean = 0;
