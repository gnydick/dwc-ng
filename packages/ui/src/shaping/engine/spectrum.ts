 ...
 *
 * THIS MODULE IS THE ABSENCE HALF of the invariant declared on `fit.ts`,
 * `one-envelope-and-it-is-fitted`: it deliberately exports no function that
 * returns a measured envelope, so no consumer can obtain one. (Declared there
 * and merely described here — an invariant with two declaration sites has two
 * mechanisms to keep in step, which is the thing it is against.)
 *
 * An ideal band mask is a zero-phase filter with a sinc impulse response
 * ~1/(2·rel·f) long. A ring-down starts abruptly at the stop, so the mask has
 * no signal on the left half of its kernel and its magnitude RISES for tens of
 * milliseconds before settling — measured 2026-08-23 on a pure decaying
 * sinusoid whose envelope cannot rise: reading/truth ran 0.35 → 1.00 → 1.52
 * for 18 Hz ζ 0.127 at rel 0.25. Padding the input with run-in does not help
 * (a zero run-in leaves the rise at 46 ms); on real captures it only replaces
 * the rise with deceleration bleeding through the band. A measured envelope of
 * a ring-down is not recoverable near the stop, so the engine does not offer
 * one, and `bandPass` hands back the SIGNAL for a chart to draw or a test to
 * compare — never an amplitude over time. See fit.ts for the measurements.
 */
