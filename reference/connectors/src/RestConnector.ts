...
export class RestConnector extends BaseConnector {
	...
	static override async connect(hostname: string, settings: Settings): Promise<BaseConnector> {
		...
		let sessionKey = null;
		try {
			const response = await BaseConnector.request("GET", `${settings.protocol}//${hostname}${settings.baseURL}machine/connect`, {
				password: settings.password
			});
			sessionKey = response.sessionKey;
		} catch (e) {
			if (!(e instanceof NetworkError) && !(e instanceof FileNotFoundError)) {
		...
		const socket = new WebSocket(`${socketProtocol}//${hostname}${settings.baseURL}machine${(sessionKey ? `?sessionKey=${sessionKey}` : "")}`);
